...
def signal_handler(sig, frame, port):
    ...
    print('\nПолучен сигнал завершения, останавливаем фронтенд...')
    ...
    print('Фронтенд корректно остановлен')
    sys.exit(0)

def main():
    """Основная функция запуска фронтенда"""
    
    # Получаем порт из переменных окружения
    port = int(os.getenv('WEB_INTERFACE_PORT', 8001))
    ...
    if not Path('db/templates').exists():
        print('Директория db/templates не найдена')
        sys.exit(1)
    ...
    print(f'Запускаем фронтенд (веб-интерфейс) на порту {port}...')
    print('Для остановки используйте Ctrl+C')
    print('=' * 50)
    
    try:
        # Запускаем Django сервер для веб-интерфейса
        ...
        if 'process' in locals():
            print('Завершаем процесс фронтенда...')
            process.terminate()
            try:
        ...
        print('Фронтенд корректно остановлен, порт освобожден')
